...
| 인증 | Spring Security + JWT (HS256) + Refresh Token | Bearer access token, refresh token rotation, BCrypt 해시 |
...
- **MCP (Model Context Protocol)**: 기업 내부 LLM이 창고 상태를 조회·제어하는 Tool·Resource 인터페이스. 기본 설정에서는 JWT 인증이 필요하며, 로컬 학습 환경에서만 설정으로 공개 가능
...
- **Idempotency**: 두 층위로 멱등성 보장
  - **예약·확정·취소**: `Reservation` 엔티티의 `reservationId`(클라이언트 제공 자연키)로 **도메인 차원 멱등** — REST·MCP 어느 진입점에서 같은 키로 재호출해도 중복 차감이 없다(`confirm`/`cancel`은 예약 전체 대상, 이미 처리된 상태면 no-op)
...
- **Authentication**: Spring Security + JWT(HS256). 로그인 시 access token과 refresh token을 발급하고, refresh token은 해시로 저장한 뒤 갱신 시 회전한다. 비밀번호는 BCrypt 해시 저장
- **Operational Exposure**: MCP, Swagger UI, GraphiQL, Actuator는 기본 설정에서 보호되며 로컬 프로필에서만 명시적으로 공개한다.
...
| **MCP** | JWT 인증 기본값 | JWT 인증 기본값 | 로컬 학습 환경에서만 설정으로 공개 가능 |

Swagger UI, GraphiQL, `/actuator/**`도 기본 설정에서는 인증이 필요합니다. `application-local.yml`에서만 `app.security.exposure.*` 설정으로 공개할 수 있습니다.
...
| Protocol | Endpoint |
| --- | --- |
| REST (인증) | `POST /api/v1/auth/signup`, `POST /api/v1/auth/login`, `POST /api/v1/auth/refresh`, `POST /api/v1/auth/logout`, `POST /api/v1/auth/logout-all` |
...
| MCP | `POST /mcp` (`getInventory`, `adjustStock`, `reserveStock`, `confirmStock`, `cancelReservation`, 기본 JWT 필요) |

- **UI**: 프론트엔드는 제공하지 않는다. 로컬 프로필에서는 Swagger UI와 GraphiQL을 공개해 테스트할 수 있다.
...
>
> access token 기본 만료는 15분(`APP_JWT_EXPIRATION_MINUTES`), refresh token 기본 만료는 14일(`APP_REFRESH_TOKEN_EXPIRATION_DAYS`)입니다.
>
> **로컬 공개 설정**
>
> 기본 설정에서는 MCP, Swagger UI, GraphiQL, Actuator가 보호됩니다. `application-local.yml.example`은 로컬 학습 편의를 위해 `app.security.exposure.mcp-public=true`, `app.security.exposure.dev-endpoints-public=true`, `spring.graphql.graphiql.enabled=true` 예시를 포함합니다.

> **인증 흐름**
>
> 재고 쓰기(`adjust`/`reserve`/`confirm`/`cancel`)는 access token Bearer 인증이 필요합니다. REST 재고 조회(GET)와 GraphQL 조회는 공개입니다.
>
> ```bash
> # 1) 회원가입
...
>
> # 3) access token 갱신. refresh token은 회전되므로 새 값을 저장해야 함
...
| 프로토콜 | 테스트 방법 |
| --- | --- |
| **REST** | 로컬 프로필 Swagger UI — http://localhost:8081/swagger-ui.html (쓰기는 우측 상단 **Authorize**에 `accessToken` 입력) |
| **GraphQL** | 로컬 프로필 GraphiQL — http://localhost:8081/graphiql (조회 전용) |
| **MCP** | `POST http://localhost:8081/mcp` (STREAMABLE). 기본 설정에서는 Bearer 인증이 필요하며, 로컬 프로필에서는 공개 설정으로 MCP Inspector나 로컬 Claude에 연결 가능 |
...
| 영역 | 기술 | 역할 | 상태 |
| --- | --- | --- | --- |
| 인증·인가 | Spring Security + JWT + Refresh Token | access token 인증, refresh token 회전, REST/MCP 보호, BCrypt | ✅ 완료 |
