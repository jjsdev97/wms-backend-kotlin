...
- **Pluggable Auth**: `AuthProvider` 인터페이스로 JWT ↔ Keycloak 전환 (Strategy Pattern)
...
| Rate Limiting | Bucket4j + Redis | Spring 필터 기반 호출 제한 |
...
| 영역 | 기술 | 역할 |
| --- | --- | --- |
| 인증·인가 | Spring Security + JWT ↔ Keycloak | `AuthProvider` 포트로 추상화, 설정값으로 전환 |
| 관측성 | Prometheus + Grafana | 메트릭 수집·대시보드 |
| 분산 추적 | Tempo + OpenTelemetry | 프로토콜별 레이턴시 비교 |
| 부하 테스트 | k6 / Gatling | 목표: 500 RPS @ p99 < 200ms. Redis `DECRBY` + DB Atomic UPDATE 조합으로 재고 차감 1,000 req/s 무결성 |

Loki, Spring Cloud Gateway는 단일 서비스 규모에서 불필요하여 제외. 서비스 분리 시 재검토.
...
| 회복 탄력성 | Resilience4j | 외부 시스템 연동(ERP·Shipping API 등) 대비 Circuit Breaker, Retry, Bulkhead |
...
| 우선순위 | 목표 | 대표 기술 |
| :---: | --- | --- |
| P1 | 기본 동작·신뢰성 | Redis, Flyway, Idempotency, Testcontainers, Bucket4j |
| P2 | 보안·관측성 | Spring Security/Keycloak, Prometheus, Grafana, Tempo, OTel, k6 |
| P3 | 비동기·대용량 | Kafka, Outbox Pattern, Resilience4j, Spring Batch |
