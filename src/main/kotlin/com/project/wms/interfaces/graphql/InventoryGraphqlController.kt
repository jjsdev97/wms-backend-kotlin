...
class InventoryGraphqlController(private val inventoryService: InventoryService) {
    ...
    @MutationMapping
    fun reserveStock(@Argument id: Long, @Argument amount: Int): InventoryResponse =
        InventoryResponse.from(inventoryService.reserve(ReservationCommand(id, amount)))

    @MutationMapping
    fun confirmStock(@Argument id: Long, @Argument amount: Int): InventoryResponse =
        InventoryResponse.from(inventoryService.confirm(ReservationCommand(id, amount)))

    @MutationMapping
    fun cancelReservation(@Argument id: Long, @Argument amount: Int): InventoryResponse =
        InventoryResponse.from(inventoryService.cancel(ReservationCommand(id, amount)))
}
